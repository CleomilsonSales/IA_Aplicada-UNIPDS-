...
async function loadModelAndLabel(){
  ...
  _label = await(await fetch(LABELS_PATH)).json();
...
/**
 * tidy garante que os tensores temporarios serão descartados automaticamente
 * evintando vazamento de memoria
 *
 * fromPixels converte imagemBitmap/ImageData [H,W,3]
...
function *processPrediction({ boxes, scores, classes }, width, height){
  for (let index = 0; index < scores.length; index++){
    ...
    const label =  _label[classes[index]];
    if (label !== 'kite') continue;

    //*4 pq é para pegar as quatros posições
    let [x1, y1, x2, y2] = boxes.slice(index * 4, (index + 1) * 4);

    //para ajustar a tela como ela estiver, ja que o modelo é apenas 640x640
    x1 *= width;
    x2 *= width;
    y1 *= height;
    y2 *= height;
